...
function froms(froms) {
  ...
  return froms.map(function (from) {
    ...
    if (isObject(from)) {
      if (isUndefined(from.target) && isUndefined(from.default)) {
        throw {
          name: 'Error',
          message: 'from.target and from.default are both undefined',
          arguments: args
        };
      }
      if (isUndefined(from.default) && !isString(from.target)) {
        throw {
          name: 'Error',
